...
(document de méthode de l'auteur, non publié dans ce dépôt). Le design est un **corpus apparié** : un seul corpus
...
|---|---|---|
| Schéma de base à jour | `db migrate` | Les migrations 001…014 sont appliquées (idempotent) |
...
4. extrait les **liens** (créés en `expressionlink`) et les médias — chaque lien est classé par
   **zone structurelle** dans `expressionlink.kind` (`body`, `nav`, `toc`, `reco`, `ref` ; la règle
   qui a tranché dans `kind_rule`, la jambe d'extraction dans `origin`). Toutes les zones sont
   **conservées en base** : le tri en réseau éditorial se fait à l'export (§15) ;
...
WHERE l.name = '$LAND'
  AND (el.kind IS NULL OR el.kind IN ('body','ref'));
"

# Audit SQL 3 : ventilation par zone structurelle — ce que les profils d'export trieront
sqlite3 -header -column "$DB" "
SELECT COALESCE(el.kind, 'body (avant migration 014)') AS zone,
       COUNT(*)                                        AS liens
FROM expressionlink el
JOIN expression s ON s.id = el.source_id
JOIN land l ON l.id = s.land_id
...
(`--force` re-soumet aussi les `non`, jamais les `oui`). Le prompt est désormais rédigé
**en anglais quelle que soit la langue du Land**, mais il **déclare la langue de travail du
projet** (ici « The project's working language is English (en) ») et demande au modèle de
raisonner dans cette langue et son contexte culturel ; le verdict stocké reste `oui`/`non`
(le parseur accepte aussi `yes`/`no`).
...
thématique. Le **mode controverse** resserre le critère : il ne **garde** que les pages
éditoriales qui *prennent position* sur la controverse du projet (une prise de position, un
argument, une opinion, une analyse, ou une information substantielle) et **rejette** les pages
d'index, de sommaire, de navigation et les présentations d'entreprise génériques qui ne
débattent pas de l'enjeu (tradition de la cartographie de controverses, Venturini/Latour). La
sémantique du verdict est inchangée — un `non` force toujours la pertinence à 0. On l'active
soit **globalement** via le réglage `openrouter_issue_mode` (variable d'env
`MWI_OPENROUTER_ISSUE_MODE`, défaut `false`) — honoré par **toutes** les gates (crawl,
readable, consolidate, `llm validate`) —, soit **ponctuellement** pour un seul run via le flag
`--issuecrawl` :
...
recalcule la pertinence **lexicale** des pages déjà crawlées. **Il respecte les verdicts LLM déjà
enregistrés** : après le recalcul lexical, une page marquée `validllm='non'` voit sa pertinence
forcée à 0 — la consolidation ne **ressuscite** jamais une page rejetée par la gate (avant ce
correctif, elle écrasait silencieusement le verdict et faisait réapparaître le bruit). Une page
`validllm='oui'` ou non jugée garde son score lexical. À utiliser **après toute modification
externe de la base** — typiquement après une session de tagging dans MyWebClient, un import, ou
une suppression manuelle — pour garantir que le graphe reflète bien le contenu.
...
>
> ⚠️ **Une exception** : sur un Land crawlé **avant** la classification structurelle des liens,
> `consolidate` est le **backfill rétrospectif** — il recalcule la sortie HTML de Trafilatura
> depuis `expression.html` (donc `--fullhtml` requis pour le plein effet) et renseigne
> `kind`/`kind_rule`/`origin` sans re-crawler. Tant qu'il n'a pas tourné, tous les `kind` sont
...
Par défaut, `consolidate` **n'appelle pas** le LLM. Le flag optionnel `--llm=true` (même idiome
que `land readable --llm=true`) **rejoue la gate OpenRouter** page par page (en respectant
`openrouter_readable_min_chars`), rafraîchit `validllm`/`validmodel`, puis applique le garde-fou
ci-dessus. Si OpenRouter n'est pas configuré, le flag est ignoré avec un avertissement et la
consolidation se poursuit sans LLM (en respectant toujours les verdicts déjà stockés). Bornez les
appels avec `--limit`/`--depth`/`--minrel`, et ajoutez `--issuecrawl` pour forcer le **mode
controverse** (§8.2) sur ce run.
...
| `nodelinkcsv` | 4 CSV : `*_pagesnodes`, `*_pageslinks` (colonne `kind` en **fin** d'en-tête, NULL rendu `body`), `*_domainnodes`, `*_domainlinks`. Avec `--fullhtml=TRUE`, émet **à la place** les 4 `*fullhtml.csv` du réseau page entière (**la Procédure A, dans MWI**) | Import igraph/networkx |
...
| `nodesjson` | Graphe **domaines** force-graph `{nodes, links}` (JSON) + `corpus` (objets `{title, urlarticle, description, published_at}` du domaine) | Visualiseur React/JS (`react-force-graph`, D3, Sigma) |
| `pagesjson` | Graphe **pages** force-graph `{nodes, links}` (JSON), `tags` tableau + `seorank` objet | Visualiseur React/JS |
...
> ⚠️ **Trois pièges d'export** : (1) le filtre de pertinence par défaut est `--minrel=1` — les
...
> le comportement actuel, le chemin exact est affiché par la commande. (3) Tout export de graphe
> (`nodelinkcsv`, `pagegexf`, `nodegexf`, `nodesjson`, `pagesjson`) est filtré par
> `--link-profile`, dont le défaut est `editorial` (= zones `body` + `ref`) ; `editorial+reco`
> ajoute les blocs de recommandation, `all` n'exclut rien. Les fichiers `*fullhtml.csv` ne sont
> **jamais** filtrés par un profil : ce sont les comparateurs page entière.
...
> 💡 **Vers la suite de l'étude** : la **Procédure A** (extraction page entière) est produite
> directement par `land export --type=nodelinkcsv --fullhtml=TRUE` — les 4 fichiers
> `*fullhtml.csv` ne subissent **aucun** filtrage par profil, c'est ce qui en fait le comparateur.
> `htmldump` reste le matériau brut pour rejouer une autre extraction hors MWI ; les
...
SELECT
  ...
  (SELECT COUNT(DISTINCT el.source_id) FROM expressionlink el
    ...
    WHERE l.name = '$LAND'
      AND (el.kind IS NULL OR el.kind IN ('body','ref')))                         AS pages_avec_lien_editorial,
  (SELECT COUNT(*) FROM expressionlink el
    JOIN expression s ON s.id = el.source_id JOIN land l ON l.id = s.land_id
    WHERE l.name = '$LAND'
      AND (el.kind IS NULL OR el.kind IN ('body','ref')))                         AS liens_editoriaux,
...
| Zone structurelle de chaque lien et règle appliquée | `expressionlink.kind`, `kind_rule`, `origin` | §7.1 |
| Correspondance des identifiants après fusion d'URLs | CSV de `land normalize --mapping-out` | §6 |
...
| `expressionlink` contient **toutes** les zones (`body`/`nav`/`toc`/`reco`/`ref`) | Le tri se fait à l'export (`--link-profile`, défaut `editorial` = body + ref). En SQL : `kind IS NULL OR kind IN ('body','ref')` — **jamais** `kind = 'body'`, qui exclut tout le graphe antérieur à la migration 014 |
...
- `README.md` — référence des pipelines (Crawl, Readable, Consolidation, Export) ;
- le document de méthode de l'auteur, dont ce tutoriel est la phase empirique (non publié dans ce dépôt).
