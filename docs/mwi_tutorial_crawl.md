...
4. enregistre tout dans la base : `expression.title`, `expression.readable`, `expression.relevance`, plus `expression.fetch_method` (qui a fourni le HTML : `aiohttp` / `curl_cffi` / `playwright` / `archive_org`), plus une ligne par lien dans `expressionlink` — chacune étiquetée par sa **zone structurelle** `kind` (`body`, `nav`, `toc`, `reco`, `ref`), la règle qui a tranché (`kind_rule`) et la jambe d'extraction qui a vu le lien (`origin` : `md`, `html`, `both`, `raw`) —, plus une ligne par média dans `media` ;
5. les URLs trouvées dans les pages sont insérées comme nouvelles expressions à `depth+1`, prêtes pour le prochain `crawl`.

> 💡 **Règle d'or de la cascade** — `expression.http_status` reflète la stratégie qui a **effectivement livré** le HTML : si `aiohttp` prend un 403 et que `curl_cffi` rattrape en 200, on enregistre `200`. Le compteur de statuts répond donc à la question qui compte — « a-t-on le contenu ? ». Le signal « la stratégie primaire a échoué » reste lisible dans `expression.fetch_method` (≠ `aiohttp`). Ce document affirmait l'inverse jusqu'au 2026-09 ; la sémantique a changé le 2026-05-08.
...
| `https://lemonde.fr/article/m%c3%a9lenchon` | `https://lemonde.fr/article/m%C3%A9lenchon` (escapes `%XX` en majuscules — RFC 3986, jamais décodés) |
...
| `path_casefold` | minuscule le **chemin** : `/Page` → `/page` | `export MWI_URL_PATH_CASEFOLD=true` |
| `trailing_slash` | `preserve` (défaut) ; `strip` retire le `/` final, **racine comprise** ; `add` l'ajoute | `export MWI_URL_TRAILING_SLASH=strip` |
...
- d'extraire les liens sortants → `expressionlink`, chacun étiqueté par sa zone structurelle `kind` (`body`, `nav`, `toc`, `reco`, `ref`) — la classification lit le HTML brut archivé de la page (`--fullhtml`) ; sans lui, tout reste `body` ;
...
> 💡 **Mode controverse (`--issuecrawl`)** — par défaut, la gate ne garde que les pages réellement liées au sujet. Avec `--issuecrawl`, elle bascule en *analyse de controverse* (tradition Venturini/Latour de cartographie des controverses) : elle ne retient que les pages **éditoriales / de prise de position** qui engagent le sujet du projet (prise de position, argument, opinion, analyse, ou information substantielle) et rejette les pages d'index, de résumé, de navigation et les présentations d'entreprise génériques. Le flag s'applique de la même façon à `land crawl`, `land readable --llm=true` et `land consolidate --llm=true`. Pour l'activer **globalement** (toutes les commandes de gate), poser `openrouter_issue_mode = True` dans `settings.py` ou `export MWI_OPENROUTER_ISSUE_MODE=true` ; le flag `--issuecrawl` force le mode pour un seul run. Verdict inchangé : `validllm="non"` force toujours `relevance=0`.

> 💡 **Langue des prompts** — les prompts envoyés au LLM (gate standard **et** mode controverse) sont rédigés **en anglais** mais indiquent explicitement la langue de travail du projet (ex. « The project's working language is French (fr) ») et demandent au modèle de raisonner dans cette langue et son contexte culturel. Le parseur de réponse accepte toujours `oui`/`non` comme `yes`/`no`.
...
| `nodelinkcsv` | 4 CSV : `pagesnodes`, `pageslinks`, `domainnodes`, `domainlinks`. `pageslinks` se termine par une colonne `kind` (zone structurelle du lien ; `body` par défaut) | Cytoscape, R/igraph |
...
WHERE el.source_id = 727
  -- `kind` : NULL vaut TOUJOURS `body` (liens antérieurs au sprint body-links),
  -- d'où le `IS NULL` — ne jamais écrire `el.kind = 'body'` seul
...
> 💡 **Astuce** : tout le SQL ici est portable vers PostgreSQL si vous migrez votre base un jour (un backend PostgreSQL est prévu, pas encore livré).
...
- 📖 Approfondir les pipelines : voir le README, section « Anti-Cloudflare cascade » et suivantes.
