...
├── utils.py               # canonicalize_url, merge_into, merge_results
...
**Consequence: dedup happens twice, on purpose (A08, 2026-09).** Because the
router's key keeps the query string and the database's key does not, two
results the router considers distinct can end up on a single `Expression` —
and `searchresultlog` is UNIQUE on `(search_query, url)`. Until this sprint
that raised an `IntegrityError` which rolled back the whole
`_persist_results` transaction: the provider quotas had already been spent
and nothing at all was stored. `_persist_results` therefore folds a second
time, keyed on the **stored** URL, reusing the same helper as the router
(`mwi.search.utils.merge_into`: providers concatenated, lowest rank kept,
title/snippet backfilled when empty), and writes the logs after the loop.
Do not "simplify" this by making the router share `url_normalizer` — that
neutrality is deliberate (§6 above) and locked by `test_18`.
